...
echo podman --log-level=debug run -d --replace -p $index:3306 --name mariadb_$case  -v /cases/$case:/var/lib/mysql:z -v /etc/localtime:/etc/localtime:ro --mount type=tmpfs,destination=$HOME/tmp --net=host -e MYSQL_ROOT_PASSWORD=root registry.redhat.io/rhel8/mariadb-103:1-234 > start_$case
podman --log-level=debug run -d --replace  -p $index:3306 --name mariadb_$case  -v /cases/$case:/var/lib/mysql:z -v /etc/localtime:/etc/localtime:ro --mount type=tmpfs,destination=$HOME/tmp --net=host -e MYSQL_ROOT_PASSWORD=root registry.redhat.io/rhel8/mariadb-103:1-234
...
echo podman exec -it mariadb_$case mysql -uroot -proot -h$hostname >> start_$case
